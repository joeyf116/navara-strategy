...
export default function FilePortalPage() {
	...
	const [shareMode, setShareMode] = useState<"my_portal" | "specific_user">(
		"my_portal",
	);
	...
	const myUploads = useMemo(
		() =>
			files.filter(
				(file) => file.uploaded_by_email.toLowerCase() === userEmail,
			),
		[files, userEmail],
	);
	const sharedWithMe = useMemo(
		() =>
			files.filter(
				(file) =>
					file.owner_email.toLowerCase() === userEmail &&
					file.uploaded_by_email.toLowerCase() !== userEmail,
			),
		[files, userEmail],
	);
	...
	const uniqueOwners = new Set(
		files.map((file) => file.owner_email.toLowerCase()),
	).size;
	...
	async function handleSubmit(event: FormEvent<HTMLFormElement>) {
		...
		if (isManager && shareMode === "specific_user" && !targetUserEmail.trim()) {
			setStatus("Enter a target user email to share this file.");
			return;
		}
			...
			<div className="space-y-6">
				<div>
					...
					<p className="text-muted-foreground">
						Secure uploads and downloads for your account
					</p>
				</div>
		...
		<div className="space-y-6">
			<div>
				<h1 className="text-2xl font-bold">
					{isManager ? "File Management" : "Client File Portal"}
				</h1>
				<p className="text-muted-foreground">
					{isManager
						? "Upload files to a specific user and review who shared each file"
						: "View your uploads and files shared directly with your account"}
				</p>
			</div>
			...
			<Card>
				<CardHeader>
					<CardTitle>
						{isManager ? "Upload / Share Files" : "Upload Files"}
					</CardTitle>
					<CardDescription>
						Drag and drop files or click to browse. Supported formats:{" "}
						{ALLOWED_EXTENSIONS.join(", ")}. Max size:{" "}
				...
				<CardContent>
					<form onSubmit={handleSubmit} className="space-y-4">
						{isManager && (
							<div className="space-y-2">
								<div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
									...
									>
										<div className="font-medium">Upload to My Portal</div>
										<div className="text-xs text-muted-foreground">
											Visible to admins and you
										</div>
									</button>
									<button
										type="button"
										onClick={() => setShareMode("specific_user")}
										className={`rounded-md border px-3 py-2 text-left text-sm transition-colors ${
											shareMode === "specific_user"
												? "border-primary bg-primary/5"
												: "border-border hover:bg-accent"
									...
									>
										<div className="font-medium">Share to Specific User</div>
										<div className="text-xs text-muted-foreground">
											Visible only to that user and admins
										</div>
									</button>
								</div>
								<Input
									aria-label="Target user email"
									placeholder="Target user email (e.g. client@company.com)"
									value={targetUserEmail}
									onChange={(e) => setTargetUserEmail(e.target.value)}
									type="email"
									disabled={shareMode !== "specific_user"}
								/>
						...
						<div
							className={`relative flex min-h-40 cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed transition-colors ${
								dragActive
									? "border-primary bg-primary/5"
			...
			<Card>
				<CardHeader>
					<CardTitle>
						{isManager ? "File Management" : "My Portal Files"}
					</CardTitle>
					<CardDescription>
						{isManager
							? "All files with recipient and shared-by details"
							: "Your uploads and files shared directly with your email"}
					</CardDescription>
				</CardHeader>
				<CardContent>
					<Table>
						<TableHeader>
							<TableRow>
								...
								<TableHead>{isManager ? "Recipient" : "Shared By"}</TableHead>
								{isManager ? <TableHead>Shared By</TableHead> : null}
							...
							{files.length === 0 ? (
								...
								files.map((file) => (
									<TableRow key={file.id}>
										...
										{isManager ? (
											<TableCell>{file.uploaded_by_email}</TableCell>
										) : null}
										<TableCell>
